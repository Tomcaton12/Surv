...
- Package:
```{r}
pacman::p_load(knitr, tidyverse, survival, ggsurvfit, gtsummary, tidycmprsk, condsurv, ezfun, lubridate, sm, sjosmooth)
```
...
par(mar=c(8
          , 4, 4, 2) + 0.1)
plot(km.fit, mark.time=T, ylab="S(t)")
...
s3 <- survreg(Surv(y)~1, dist="weibull")
plot(km.fit.ovarian, ylab=expression(hat(S)*"t"), xlab="temps")
lines(y=seq(0.01,0.99,0.01),x=predict(s3,newdata=data.frame(x=1),type='quantile', p=1-seq(0.01,0.99,0.01)), type="l",col=4,lwd=2)
