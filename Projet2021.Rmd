...
km.fit.ovarian.plot <- survfit(Surv(I(time/30.25), status) ~ 1, data = kaposi)

# "event" plots cumulative events (f(y) = 1-y
ggsurvplot(km.fita, fun = "event", risk.table = TRUE, surv.scale = "percent", pval = TRUE, conf.int = TRUE) 

# "cumhaz" plots the cumulative hazard function (f(y) = -log(y))
ggsurvplot(km.fita, fun = "cumhaz", risk.table = TRUE, pval = TRUE)

# "pct" for survival probability in percentage
ggsurvplot(km.fita, fun = "pct", risk.table = TRUE, pval = TRUE, conf.int = TRUE)
